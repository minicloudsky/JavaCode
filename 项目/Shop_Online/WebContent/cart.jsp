...
<script type="text/javascript">
	function delProFromCart(pid) {
		if (confirm("你确定删除该项?")) {
			location.href = "${pageContext.request.contextPath }/productServlet?method=delProFromCart&pid="
					+ pid;
		}
	}
	function clearCart() {
				...
				<div class="row">
					...
					<div class="col-lg-4" style="margin-top: 70px">
						...
						<c:if test="${empty user }">
							<p>登录后将显示您之前加入的商品</p>
							<a
								href="${pageContext.request.contextPath }/userServlet?method=login"
								class="btn btn-primary">立即登录</a>
						</c:if>
						<a href="Default.jsp" class="btn btn-primary">马上去购物</a>
					</div>
				</div>
			</div>
		</div>
							...
							<c:forEach items="${cart.cartItems }" var="entry">
								<tr class="active">
									...
									<td><a href="javascript:;"
										onclick="delProFromCart('${entry.key}')" class="delete">删除</a>
									</td>
								</tr>
							</c:forEach>
						</tbody>
			...
			<div style="margin-right: 130px;">
				...
				<div
					...
					<a href="javascript:;" onclick="clearCart();" id="clear"
						...
						href="${pageContext.request.contextPath }/productServlet?method=submitOrder">
