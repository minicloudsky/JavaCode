...
<link rel="stylesheet" type="text/css"
	href="${pageContext.request.contextPath}/css/emoticon.css" />
			...
			<div class="row-fluid">
				<div class="span2">
					<table style="width: 100%;" cellpadding="5px;">
						<tr>
							<td>★楼主&nbsp;<a href="#"
								style="font-size: 9pt; color: black;"><strong>${topic.user.nickName }</strong></a>
							</td>
						</tr>
						<tr>
							<td><c:choose>
									<c:when
										test="${(topic.user.face==null||topic.user.face=='')&&topic.user.sex=='男'}">
										<img alt=""
											src="${pageContext.request.contextPath}/images/user/user0.gif"
											style="width: 100px; height: 100px;">
									</c:when>
									<c:when
										test="${(topic.user.face==null||topic.user.face=='')&&topic.user.sex=='女'}">
										<img alt=""
									...
									<c:otherwise>
										<img alt=""
											src="${pageContext.request.contextPath}/${topic.user.face}"
											style="width: 100px; height: 100px;">
									</c:otherwise>
								</c:choose></td>
						</tr>
						<tr>
							<td>性别：${topic.user.sex }</td>
						</tr>
						<tr>
							<td>邮箱：${topic.user.email }</td>
						</tr>
						<tr>
							<td><c:choose>
									<c:when
										test="${topic.user.sectionList.size()==0&&topic.user.type!=2 }">
				...
				<div class="span10">
					<table style="width: 100%;">
						<tr style="height: 50px;">
							<td>【主题】:<strong>${topic.title }</strong>
							</td>
						</tr>
						<tr>
							<td style="text-align: right;">发表时间:『${topic.publishTime }』
							</td>
						</tr>
						<tr>
							<td>【内容】:
								<div
									style="width: 982px; padding: 6px; background-color: #F0F0F0"
									class="show_e">${topic.content }</div>
							</td>
						</tr>
					</table>
				</div>
			...
			<c:forEach items="${replyList }" var="reply" varStatus="status">
				<div class="row-fluid" style="margin-top: 20px;">
					<div class="span2">
						<table style="width: 100%;" cellpadding="5px;">
							<tr>
								<td>▲${(page-1)*10+status.index+1 }楼</td>
							</tr>
							<tr>
								<td><c:choose>
										<c:when
											test="${(reply.user.face==null||reply.user.face=='')&&reply.user.sex=='男'}">
											<img alt=""
												src="${pageContext.request.contextPath}/images/user/user0.gif"
												style="width: 100px; height: 100px;">
										</c:when>
										<c:when
											test="${(reply.user.face==null||reply.user.face=='')&&reply.user.sex=='女'}">
											<img alt=""
												src="${pageContext.request.contextPath}/images/user/female.gif"
												style="width: 100px; height: 100px;">
										</c:when>
										<c:otherwise>
											<img alt=""
												src="${pageContext.request.contextPath}/${reply.user.face}"
												style="width: 100px; height: 100px;">
										</c:otherwise>
									</c:choose> <a href="#" style="font-size: 9pt; color: black;"><strong>${reply.user.nickName }</strong></a>
								</td>
							</tr>
							<tr>
								<td>性别：${reply.user.sex }</td>
							</tr>
							<tr>
								<td>邮箱：${reply.user.email }</td>
							</tr>
							<tr>
								<td><c:choose>
										<c:when
											test="${reply.user.sectionList.size()==0&&reply.user.type!=2 }">
											<font style="color: black;">普通用户</font>
										</c:when>
										<c:when
											test="${reply.user.sectionList.size()!=0&&reply.user.type!=2 }">
								...
								<td style="text-align: right;"><c:choose>
										<c:when test="${currentUser.id==section.master.id }">
											<button class="btn btn-danger"
												onclick="javascript:deleteReply(${reply.id })">删除</button>
										</c:when>
									...
									</c:choose> 回复时间:『${reply.publishTime }』</td>
							</tr>
							<tr>
								<td>
									<div
										style="width: 982px; padding: 6px; background-color: #F0F0F0"
										class="show_e">${reply.content }</div>
								</td>
							</tr>
						</table>
					</div>
						...
						<form id="replyForm" class="form-horizontal"
							...
							<td>
								...
								<tr>
									...
									<td><textarea name="reply.content" id="Content" cols="50"
											style="height: 200px; width: 800px;"></textarea></td>
								</tr>
								<tr>
									<td><input id="userId" name="reply.user.id"
										value="${currentUser.id }" type="hidden" /> <input
										id="topicId" name="reply.topic.id" value="${topic.id }"
										type="hidden" /></td>
									<td>
										<Button class="btn btn-primary " data-dismiss="modal"
											aria-hidden="true" type="button"
											onclick="javascript:saveReply()">提交</Button> <font id="error"></font>
									</td>
								</tr>
							</table>
						</form>
