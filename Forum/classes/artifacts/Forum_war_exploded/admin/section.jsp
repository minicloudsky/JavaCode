	...
	<div class="container-fluid">
		<div id="tooBar" style="padding: 10px 0px 0px 10px;">
			<button class="btn btn-primary" type="button" data-backdrop="static"
				data-toggle="modal" data-target="#dlg" onclick="return openAddDlg()">添加小板块</button>
			&nbsp;&nbsp;&nbsp;&nbsp; <a href="#" role="button"
				class="btn btn-danger" onclick="javascrip:deleteSections()">批量删除</a>
			<form action="Section_list.action" method="post" class="form-search"
				style="display: inline;">
				&nbsp;小板块名称： <input name="s_section.name" value="${s_section.name }"
					type="text" class="input-medium search-query"
					placeholder="输入小板块名称..." /> &nbsp;所属大板块： <select
					name="s_section.zone.id"><option value="">请选择...</option>
					<c:forEach var="zone" items="${zoneList }">
						<option value="${zone.id }"
							${s_section.zone.id==zone.id?'selected':'' }>${zone.name }</option>
					</c:forEach>
				</select> &nbsp;版主： <select name="s_section.master.id"><option
						value="">请选择...</option>
					<c:forEach var="master" items="${masterList }">
						<option value="${master.id }"
							${s_section.master.id==master.id?'selected':'' }>${master.nickName }</option>
					</c:forEach>
				</select> &nbsp;
								...
								<c:forEach items="${sectionList }" var="section">
									<tr>
										...
										<td
											...
											src='${pageContext.request.contextPath}/${section.logo }'></img>
										</td>
										...
										<td style="text-align: center; vertical-align: middle;">
											<button class="btn btn-info" type="button"
												...
												data-target="#dlg"
												onclick="return modifySection(${section.id},'${section.name }',${section.zone.id },'${section.master.nickName }','${section.logo }')">修改
											</button>&nbsp;&nbsp;
											<button class="btn btn-danger" type="button"
												onclick="javascript:sectionDelete(${section.id})">删除</button>
										</td>
									</tr>
								</c:forEach>
							</tbody>
		...
		<div id="dlg" class="modal hide fade" tabindex="-1" role="dialog"
			...
			<div class="modal-body">
				<form id="fm" action="Section_save.action" method="post"
					enctype="multipart/form-data">
					<table>
						<tr>
							<td><label class="control-label" for="sectionName">请输入小板块名称：</label>
							</td>
							<td><input id="sectionName" type="text" name="section.name"
								placeholder="请输入…"></td>
						</tr>
						<tr>
						...
						<tr>
							<td><label class="control-label" for="logo">上传logo：</label>
							</td>
							<td><input type="file" id="logo" name="logo"></td>
						</tr>
						<tr>
							<td><label class="control-label" for="zone">请选择所属大板块：</label>
							</td>
							<td><select id="zone" name="section.zone.id"><option
										value="">请选择...</option>
									<c:forEach var="zone" items="${zoneList }">
						...
						<tr>
							<td><label class="control-label" for="masterNickName">版主：</label>
							</td>
							<td><input id="masterNickName" type="text"
								name="section.master.nickName"
								onkeydown="javascript:searchUserByNickName(this.value)"
								placeholder="请输入昵称回车"> <font id="info"
								style="color: red;"></font></td>
						</tr>
					</table>
					<input id="id" type="hidden" name="section.id"> <input
						id="masterId" type="hidden" name="section.master.id">
				</form>
			</div>
