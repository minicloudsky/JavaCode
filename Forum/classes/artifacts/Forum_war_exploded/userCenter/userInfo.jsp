		...
		<table class="table table-bordered table-striped with-check">
			...
			<tr>
				...
				<td style="text-align: center; vertical-align: middle;"><c:choose>
						<c:when
							test="${(currentUser.face==null||currentUser.face=='')&&currentUser.sex=='男'}">
							<img alt=""
								src="${pageContext.request.contextPath}/images/user/user0.gif"
								style="width: 100px; height: 100px;">
						</c:when>
						<c:when
							test="${(currentUser.face==null||currentUser.face=='')&&currentUser.sex=='女'}">
							<img alt=""
						...
						<c:otherwise>
							<img alt=""
								src="${pageContext.request.contextPath}/${currentUser.face}"
								style="width: 100px; height: 100px;">
						</c:otherwise>
					</c:choose></td>
				...
				<td style="text-align: center; vertical-align: middle;"><c:choose>
						<c:when
							test="${currentUser.sectionList.size()==0&&currentUser.type!=2 }">
						...
						<c:when
							...
							<font style="color: blue;">版主</font>
					                       【<c:forEach
								items="${currentUser.sectionList }" var="section">
					            ${section.name }；
					         </c:forEach>】
					    </c:when>
						<c:otherwise>
				...
				<td style="text-align: center; vertical-align: middle;"><a
					class="btn btn-info" type="button" href="User_preSave.action">修改</a>&nbsp;&nbsp;
				</td>
			</tr>
		</table>
	</div>
