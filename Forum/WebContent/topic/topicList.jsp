...
<link rel="stylesheet"
	href="${pageContext.request.contextPath}/admin/css/bootstrap.css" />
<link rel="stylesheet"
...
<script
	src="${pageContext.request.contextPath}/admin/js/jquery.dataTables.min.js"></script>
<%-- <script src="${pageContext.request.contextPath}/admin/js/unicorn.js"></script> --%>
<script
	src="${pageContext.request.contextPath}/admin/js/unicorn.tables.js"></script>
<script type="text/javascript">
...
<body>
	...
	<div style="width: 1200px; margin: 0 auto;">
		<div style="margin-bottom: 10px;">
			<a class="" href="Topic_preSave.action?sectionId=${section.id }"><img
				alt="发帖" src="${pageContext.request.contextPath}/images/post.jpg"></a>
			<div class="pagination alternate pull-right" align="center"
				style="margin: 0px;">
						...
						<c:forEach items="${zdTopicList }" var="topic">
							<tr>
								...
								<td style="text-align: center; vertical-align: middle;"><a
									href="Topic_details.action?topicId=${topic.id }">${topic.title }</a>
								</td>
								<td style="text-align: center; vertical-align: middle;">
									${topicReplyCount.get(topic) }</td>
								<td style="text-align: center; vertical-align: middle;">
								...
								<td style="text-align: center; vertical-align: middle;"><c:choose>
										<c:when
											test="${currentUser.id==topic.user.id&&currentUser.id!=section.master.id }">
											<button class="btn btn-info" data-backdrop="static"
												onclick="#">修改</button>
										</c:when>
										<c:when test="${currentUser.id==section.master.id }">
											<button class="btn btn-info" type="button"
												...
												data-target="#dlg"
												onclick="return modifyTopic(${topic.id },${topic.top },${topic.good })">修改</button>
											<button class="btn btn-danger"
												onclick="javascript:deleteTopic(${topic.id })">删除</button>
										</c:when>
	...
	<div id="dlg" class="modal hide fade" tabindex="-1" role="dialog"
		...
		<div class="modal-body">
			<form id="fm" action="#" method="post" enctype="multipart/form-data">
				<table>
					<tr>
						<td><label class="control-label" for="top">置顶：</label></td>
						<td><select id="topicTop">
								<option value="0">非置顶</option>
								<option value="1">置顶</option>
						</select></td>
					</tr>
					<tr>
						<td><label class="control-label" for="good">精华：</label></td>
						<td><select id="topicGood">
