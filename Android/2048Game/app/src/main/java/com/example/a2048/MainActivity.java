    ...
    public boolean onTouch(View v, MotionEvent event) {
        switch (event.getAction()) {
            ...
            case MotionEvent.ACTION_UP:
                if (x2 - x1 > 100 && Math.abs(x2 - x1) > Math.abs(y2 - y1)) {
...
//                    Toast.makeText(this,"向左滑动",Toast.LENGTH_SHORT).show();
                    moveToLeft();
                } else if (y2 - y1 > 100 && Math.abs(x2 - x1) < Math.abs(y2 - y1)) {
...
//                btn.setBackgroundColor(0xfff65e3b);
    ...
    public void saveBest_score(Score score){
        SharedPreferences.Editor editor =getSharedPreferences("score_data",MODE_PRIVATE).edit();
        editor.putString("name",score.getName());
        editor.putInt("score",score.getScore());
        editor.putString("time",score.getTime().toString());
        editor.apply();
    }
